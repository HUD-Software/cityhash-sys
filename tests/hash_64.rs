...
mod impl_for_u8 {
    ...
    use crate::CITY_HASH_64_WITH_SEED_RESULT;
    use cityhash_sys::CityHash;

    #[test]
    fn hash_64_from_null_ptr() {
        assert_eq!(
            unsafe { core::slice::from_raw_parts(core::ptr::null::<u8>(), 0) }.city_hash_64(),
            0x9AE16A3B2F90404Fu64
        );
    }
    ...
    fn hash_64_with_seed_from_null_ptr() {
        assert_eq!(
            unsafe { core::slice::from_raw_parts(core::ptr::null::<u8>(), 0) }
                .city_hash_64_with_seed(0x5555555555555555),
            0x843a9b9c18a444a2
        );
    }
    ...
    fn hash_64_with_seeds_from_null_ptr() {
        assert_eq!(
            unsafe { core::slice::from_raw_parts(core::ptr::null::<u8>(), 0) }
                .city_hash_64_with_seeds(0x5555555555555555, 0xAAAAAAAAAAAAAAAA),
            0xc1ebecd2e7eb66d
        );
    }
    ...
    fn hash_64_no_seed() {
        ...
        for index in 0..=255u8 {
            key[index as usize] = index;
            assert_eq!(
                key[0..index as usize].city_hash_64(),
                CITY_HASH_64_RESULTS[index as usize]
            );
        }
    }
    ...
    fn hash_64_with_seed() {
        ...
        for index in 0..=255u8 {
            key[index as usize] = index;
            assert_eq!(
                key[0..index as usize].city_hash_64_with_seed(index as u64),
                CITY_HASH_64_WITH_SEED_RESULT[index as usize]
            );
        }
    }
    ...
    fn hash_64_with_seeds() {
        ...
        for index in 0..=255u8 {
            key[index as usize] = index;
            assert_eq!(
                key[0..index as usize].city_hash_64_with_seeds(index as u64, 0xAAAAAAAAAAAAAAAA),
                CITY_HASH_64_WITH_SEEDS_RESULT[index as usize]
            );
        }
    }
...
mod impl_for_str {
    ...
    #[test]
    fn hash_64_from_str_lipsum() {
        assert_eq!(lipsum::LIPSUM.city_hash_64(), 0x999ac74addcd6dc7u64);
    }

    #[test]
    fn hash_64_with_seed_from_str_lipsum() {
        assert_eq!(
            lipsum::LIPSUM.city_hash_64_with_seed(0x5555555555555555),
            0x6976f8f604619566
        );
    }
    ...
    fn hash_64_with_seeds_from_str_lipsum() {
        assert_eq!(
            lipsum::LIPSUM.city_hash_64_with_seeds(0x5555555555555555, 0xAAAAAAAAAAAAAAAA),
            0x716f9c1db5c27122
        );
    }
}
